...
async function getBadgeCounts(cookie: string | undefined, isSeller: boolean) {
  if (!cookie) return { cart: 0, notifications: 0, messages: 0, orders: 0 };

  const [cart, notifications, messages, orders] = await Promise.all([
    ...
    apiFetch<{ rooms: { unread_count: number }[] }>('/chats/rooms', { cookie, cache: 'no-store' })
      ...
      .catch(() => 0),
    getPendingOrderCount(cookie, isSeller),
  ]);

  return { cart, notifications, messages, orders };
}
    ...
    <div className={styles.shell}>
      ...
      <div className={styles.body}>
        <SidebarShell role={user.current_role}>
          <SidebarNav cartCount={cart} messageCount={messages} ordersCount={orders} isSeller={isSeller} />
        </SidebarShell>

        <main className={styles.main}>{children}</main>
      </div>

      <BottomNav cartCount={cart} messageCount={messages} ordersCount={orders} isSeller={isSeller} />
    </div>
  );
}
